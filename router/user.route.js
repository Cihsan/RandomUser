...
router.get("/random",  (req, res) => {
  const users = fs.readFileSync(path.join(__dirname, '../info/userData.json'), 'utf8');
  ...
  router.post("/save", (req, res) => {
    const users = fs.readFileSync("../info/userData.json", "utf-8");
    const user = JSON.parse(users);
    const newUser = req.body;
    if (newUser.id && newUser.name && newUser.contact && newUser.address && newUser.photoUrl && newUser.gender) {
      user.push(newUser);
      fs.writeFileSync("../info/userData.json", JSON.stringify(user));
      res.send(user);
    } else {
  ...
  app.patch("/bulk-update", (req, res) => {
   ...
   if(ids && updatedData){
    ...
    fs.writeFileSync("../info/userData.json", JSON.stringify(updatedUser));
